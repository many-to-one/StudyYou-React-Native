...
const WatchTowerLector = ({day, week_ago, navigation}) => {
  ...
  const getUsers = async() => {
    ...
    const resp = await fetch(`${proxy}/users/users_by_lector/${datas.congregation}/`)
  ...
  if(stuff === true){
    return(
      <View>
        ...
        <View>
          {dateWatchTowerLector.map((person, index) => (
            <ShowStuff 
            key={person.id}
            person={person}
            USERS={USERS}
