...
const WeekReadingBible = ({day, week_ago, navigation}) => {
  ...
  const getCalendarDatesByDate = async() => {
    let datas = JSON.parse(await AsyncStorage.getItem("asyncUserData"))
    const body = {'date': day, 'action': 'BibleReading', 'congregation': datas.congregation}
  ...
  const setWeekReadingBible = async(selected) => {
    let datas = JSON.parse(await AsyncStorage.getItem("asyncUserData"))
      for(let k in USERS){  
        if(selected === USERS[k]){
  ...
  return(
    <View>
      <View style={styles.row}>
        ...
        />
        <TalkBtn onPress={() => setWeekReadingBible(selected)}/>
      </View>
      <View>
        {dateWeekReadingBible.map((person, index) => (
          <ShowStuff 
          key={person.id}
          person={person}
          USERS={USERS}
...
//       if(e.date === day && e.action === 'BibleReading'){  
...
//                   size={20} 
//                   color={'white'} 
//                   onPress={() => deleteWeekReadingBible(e)}     
//                   />
//           </View>  
...
export default WeekReadingBible
